...
interface Row {
  id: number; claimNumber: string; type: string;
...
interface EligibleRow { id: number; customerName: string | null; packageName: string | null; amount: string; salesName?: string | null; createdAt: string }
interface PartnerLite { id: number; name: string }

const CLAIM_TYPES = ["payment", "partner_commission", "sales_commission"] as const;
...
function ManualClaimBuilder({ onCreated }: { onCreated: () => void }) {
  ...
  const [type, setType] = useState<ClaimType>("partner_commission");
  ...
  const eligibleQ = useQuery({
    queryKey: ["claim-eligible", type, partnerId],
    queryFn: () => api<EligibleRow[]>(`/api/claims/eligible?type=${type}&partnerId=${partnerId}`),
    enabled: !!partnerId,
  });
  const create = useMutation({
    mutationFn: () => api<{ id: number }>("/api/claims", {
      method: "POST",
      json: { type, partnerId: partnerId || undefined, itemIds: Array.from(selected), notes: notes || undefined },
    }),
  ...
  return (
    <div className="stamp-card p-4 mb-4">
      ...
      <div className="grid md:grid-cols-3 gap-3 mb-3 items-end">
        <div>
          <label className="text-xs text-muted block mb-1">{t("common.type")}</label>
          <select className="input" value={type} onChange={(e) => { setType(e.target.value as ClaimType); setSelected(new Set()); }}>
            {CLAIM_TYPES.map((tt) => <option key={tt} value={tt}>{t(`financial.claimTypes.${tt}`)}</option>)}
          </select>
        </div>
        {isCompany && (
          <div>
...
export function ClaimsPage() {
  ...
  const list = useQuery({
    queryKey: ["claims", status, type],
    queryFn: () => {
      const qs = new URLSearchParams();
      if (status) qs.set("status", status);
      if (type) qs.set("type", type);
      return api<Row[]>(`/api/claims?${qs.toString()}`);
    },
  });
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap gap-2 items-center">
        ...
        <select className="input ml-auto" value={status} onChange={(e) => setStatus(e.target.value)}>
          ...
          <tbody>
            ...
            {list.data?.map((r) => (
              <tr key={r.id}>
                ...
                <td><span className="pill-violet">{t(`financial.claimTypes.${r.type}`)}</span></td>
                <td>{r.partnerName}</td>
...
interface ClaimDetail {
  claim: {
    id: number; claimNumber: string; type: string; partnerId: number; status: string;
  ...
  items: Array<{ id: number; childId: number | null; amount: string; customerName: string | null; packageName: string | null; salesName?: string | null }>;
