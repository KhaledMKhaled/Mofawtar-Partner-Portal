...
async function fetchReport(key: ReportKey, partnerId?: number, from?: string, to?: string): Promise<{ headers: string[]; rows: Record<string, unknown>[] }> {
  ...
  switch (key) {
    case "payments_summary": {
      const filters: any[] = [...dateFilter(orderPayments.createdAt)];
    ...
    case "partner_commissions_summary": {
      const filters: any[] = [...dateFilter(partnerCommissions.createdAt)];
      ...
      const rows = where ? await q.where(where).orderBy(desc(partnerCommissions.createdAt)).limit(2000) : await q.orderBy(desc(partnerCommissions.createdAt)).limit(2000);
      return { headers: ["id","partner","customer","package","baseAmount","pct","amount","status","createdAt"], rows };
    }
    case "sales_commissions_summary": {
      const filters: any[] = [...dateFilter(salesCommissions.createdAt)];
      ...
      const rows = where ? await q.where(where).orderBy(desc(salesCommissions.createdAt)).limit(2000) : await q.orderBy(desc(salesCommissions.createdAt)).limit(2000);
      return { headers: ["id","partner","sales","customer","package","pct","amount","status","createdAt"], rows };
    }
    case "claims_summary": {
      const filters: any[] = [...dateFilter(claims.createdAt)];
      ...
      const rows = where ? await q.where(where).orderBy(desc(claims.createdAt)).limit(2000) : await q.orderBy(desc(claims.createdAt)).limit(2000);
      return { headers: ["id","claimNumber","partner","status","totalAmount","autoGenerated","createdAt"], rows };
    }
    case "requests_summary": {
      const filters: any[] = [...dateFilter(requests.createdAt)];
      ...
      const rows = where ? await q.where(where).orderBy(desc(requests.createdAt)).limit(2000) : await q.orderBy(desc(requests.createdAt)).limit(2000);
    ...
    case "ownership_summary": {
      ...
      const rows = where ? await q.where(where).orderBy(desc(customerOwnership.createdAt)).limit(2000) : await q.orderBy(desc(customerOwnership.createdAt)).limit(2000);
...
reportsRouter.get("/:key", requirePerm("reports:view"), async (req, res) => {
  ...
  const partnerId = partnerScoped(cu) ? cu.partnerId! : (req.query.partnerId ? Number(req.query.partnerId) : undefined);
  const from = (req.query.from as string) || undefined;
  const to = (req.query.to as string) || undefined;
  const data = await fetchReport(key, partnerId, from, to);
  res.json(data);
});
