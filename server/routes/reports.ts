  ...
  switch (key) {
    case "payments_summary": {
      ...
      const q = db.select({
        ...
        net: financialItems.netAmountDueToCompany,
    ...
    case "partner_commissions_summary": {
      ...
      const q = db.select({
        ...
        customerName: customers.name,
        packageName: packages.name,
        baseAmount: financialItems.commissionBase,
        pct: financialItems.partnerCommissionPercentage,
    ...
    case "sales_commissions_summary": {
      ...
      const q = db.select({
        ...
        packageName: packages.name,
        amount: financialItems.amount,
        pct: financialItems.salesCommissionPercentage,
        status: financialItems.status,
        createdAt: financialItems.createdAt,
      }).from(financialItems)
        .leftJoin(partners, eq(partners.id, financialItems.relatedPartnerId))
    ...
    case "claims_summary": {
      ...
      const q = db.select({
        ...
        type: claims.type, createdAt: claims.createdAt,
      ...
      const rows = raw.map((r) => ({ id: r.id, claimNumber: r.claimNumber, partner: r.partnerName, status: r.status, totalAmount: r.totalAmount, type: r.type, createdAt: r.createdAt }));
      return { headers: ["id","claimNumber","partner","status","totalAmount","type","createdAt"], rows };
    }
...
async function dashboardKpisHandler(req: import("express").Request, res: import("express").Response) {
  ...
  // Greenfield: derive everything from the unified financial_items ledger.
  const fiPartnerFilter = scoped ? sql`related_partner_id = ${cu.partnerId}` : sql`TRUE`;

  const paymentsAgg = await exec1<{ pending_count: string; net_total: string }>(sql`
    SELECT COUNT(*) FILTER (WHERE status <> 'settled' AND is_voided = FALSE)::text AS pending_count,
           COALESCE(SUM(amount) FILTER (WHERE status <> 'settled' AND is_voided = FALSE),0)::text AS net_total
    FROM financial_items WHERE type = 'payment_item' AND ${fiPartnerFilter}`);

  const pcAgg = await exec1<{ safety_total: string; eligible_total: string; eligible_count: string }>(sql`
    SELECT COALESCE(SUM(CASE WHEN status = 'not_added_to_claim' AND is_claimable = FALSE THEN amount ELSE 0 END),0)::text AS safety_total,
           COALESCE(SUM(CASE WHEN status = 'not_added_to_claim' AND is_claimable = TRUE THEN amount ELSE 0 END),0)::text AS eligible_total,
           COUNT(*) FILTER (WHERE status = 'not_added_to_claim' AND is_claimable = TRUE)::text AS eligible_count
    FROM financial_items WHERE type = 'partner_commission_item' AND is_voided = FALSE AND ${fiPartnerFilter}`);

  const scAgg = await exec1<{ open_total: string; paid_ytd: string }>(sql`
    SELECT COALESCE(SUM(CASE WHEN status <> 'settled' THEN amount ELSE 0 END),0)::text AS open_total,
           COALESCE(SUM(CASE WHEN status = 'settled' AND settled_at >= date_trunc('year', NOW()) THEN amount ELSE 0 END),0)::text AS paid_ytd
    FROM financial_items WHERE type = 'sales_commission_item' AND is_voided = FALSE AND ${fiPartnerFilter}`);

  const claimsPartnerFilter = scoped ? sql`partner_id = ${cu.partnerId}` : sql`TRUE`;
  const claimsAgg = await exec1<{ open_count: string }>(sql`
    SELECT COUNT(*) FILTER (WHERE status = 'draft')::text AS open_count
    FROM claims WHERE ${claimsPartnerFilter}`);

  const settlAgg = await exec1<{ ready_total: string }>(sql`
    SELECT COALESCE(SUM(amount) FILTER (WHERE status = 'added_to_settlement'),0)::text AS ready_total
    FROM financial_items WHERE type = 'partner_commission_item' AND is_voided = FALSE AND ${fiPartnerFilter}`);
  ...
  const revAgg = await exec1<{ revenue_month: string }>(sql`
    SELECT COALESCE(SUM(gross_customer_amount) FILTER (WHERE created_at >= date_trunc('month', NOW())),0)::text AS revenue_month
    FROM financial_items WHERE type = 'payment_item' AND is_voided = FALSE AND ${fiPartnerFilter}`);
