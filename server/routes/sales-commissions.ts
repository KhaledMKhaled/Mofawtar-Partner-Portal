...
salesCommissionsRouter.get("/", requirePerm("sales_commissions:view"), async (req, res) => {
  ...
  const rows = await db.select({ id: financialItems.id, requestId: financialItems.relatedRequestId, srNumber: requests.srNumber, partnerId: financialItems.relatedPartnerId, partnerName: partners.name, salesUserId: financialItems.relatedSalesUserId, salesName: users.name, customerName: customers.name, packageName: packages.name, baseAmount: financialItems.commissionBase, pct: financialItems.salesCommissionPercentage, salesCommissionAmount: financialItems.salesCommissionAmount, amount: financialItems.amount, status: financialItems.status, eligibleForClaimAt: financialItems.eligibleForClaimAt, createdAt: financialItems.createdAt }).from(financialItems).leftJoin(requests, eq(requests.id, financialItems.relatedRequestId)).leftJoin(customers, eq(customers.id, financialItems.relatedCustomerId)).leftJoin(partners, eq(partners.id, financialItems.relatedPartnerId)).leftJoin(packages, eq(packages.id, financialItems.relatedPackageId)).leftJoin(users, eq(users.id, financialItems.relatedSalesUserId)).where(and(...filters)).orderBy(desc(financialItems.createdAt)).limit(500);
  res.json(rows);
});
